...
export function setupKeyboard() {
  window.addEventListener("keyup", (e) => {
    ...
    const pressedKey = e.key == "*" ? "x" : e.key == "Enter" ? "=" : e.key;

    if (numbers.includes(pressedKey)) {
      hundleNumber(pressedKey);
    } else if (operations.includes(pressedKey)) {
      hundleOperation(pressedKey);
    } else if (pressedKey == "Delete") hundleFuncs("reset");
    else if (pressedKey == "Backspace") hundleFuncs("del");
  });
}
